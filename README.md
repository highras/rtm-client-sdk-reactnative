# fpnn rtm sdk reactnative #

#### 使用示例

```javascript

import rtm from 'path/to/rtm-reactnative-sdk/rtm.js';
import fpnn from 'path/to/rtm-reactnative-sdk/fpnn.js';

let client = new rtm.RTMClient({ 
    ssl_endpoint: 'RTM_ENDPOINT', // 从云上曲率官网控制台获取: `WebSocket SSL接入点`
    autoReconnect: true,
    connectionTimeout: 10 * 1000,
    pid: 11000001 // 从云上曲率官网控制台获取项目ID
    // platformImpl: new fpnn.WechatImpl(),  // 导出微信平台需指定平台类型
});
...
// 登录，请注意uid为int64类型，需要使用rtm.RTMConfig.Int64封装，避免js超长数字截断
...
// 监听不同类型的消息推送，pushName种类可参考RTMConfig中定义的枚举
